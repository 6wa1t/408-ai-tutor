...
6. 数学公式使用LaTeX格式（$...$）
7. **识别页面中的插图/图表/代码截图**：如果某道题下方（或旁边）附有插图，请估算该图在页面上的**垂直位置范围**，记录在 `image_y_range` 中（格式：[上边界比例, 下边界比例]，范围0~1，0=页面顶部，1=页面底部）
   - 例如：一道题的内容在页面30%~45%处，其配图在45%~60%处 → `"image_y_range": [0.45, 0.60]`
   - 如果该题没有配图，留 `null`
...
{{
  ...
  "questions": [
    {{
      ...
      "image_y_range": [0.45, 0.60]
    }}
  ]
}}

如果某题有配图但你不确定精确边界，请宽估（宁大勿小），系统会自动裁剪。
