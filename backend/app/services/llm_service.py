...
VLM_QUESTION_EXTRACT_PROMPT = """你是408考研题库OCR助手。请仔细阅读此PDF页面图片，提取所有题目信息。

【页面过滤规则】
首先判断本页面是否值得提取，以下类型的页面请直接跳过，返回空结果：
- 空白页（无实质内容）
- 目录页、索引页
- 纯广告页、公众号推广页、水印页
- 仅含章节标题而无题目的过渡页
- 与题库无关的内容（前言、出版说明、使用指南等）

判断方法：如果页面文字极少（少于50字）且无题目编号，大概率属于上述类型。

【去重规则】
如果某道题与前一页末尾的题目内容高度重复（题干相同或仅排版不同），
请不要重复提取，只保留一份。跨页的题目（题干在一页末尾、选项在下一页开头）
应合并为一道题，在下一页补全选项即可，不要拆成两道题。

【提取要求】
1. 提取每道题的题号、题干完整文本、选项(A/B/C/D/E)
...
6. 数学公式使用LaTeX格式（$...$），确保上下标、求和、积分等完整准确
   - 代码片段使用代码围栏（```语言 ... ```）
   - **重要**：代码围栏结束后（```），必须留一个空行再继续写后续正文，否则渲染会出错

{context_hint}

严格返回以下JSON格式，不要有其他内容：
{{
  "page_type": "question|blank|toc|chapter_title|ad|other",
  "context": {{
    "chapter": "第X章 章名(如有)",
    "section": "X.X 小节名(如有)"
  }},
...
跳过页面时返回：
{{"page_type": "blank", "context": {{}}, "questions": []}}
...
STRUCTURED_ANSWER_SYSTEM_PROMPT = """You are a careful 408 exam multiple-choice solver.
...
Required JSON shape:
{
  ...
  "analysis_md": "Brief markdown explanation.",
  "key_points": ["knowledge point"],
  "uncertainty_reason": "Why confidence is not high, or empty string."
}

Rules:
- answer must be exactly one of A, B, C, D, or E.
- confidence must be a number from 0 to 1.
- usable_for_grading should be true only when the answer is reliable enough for automatic grading.
- If the prompt lacks enough information, provide your best candidate answer, lower confidence, and set usable_for_grading to false."""
...
class LLMService:
    ...
    def answer_question_with_confidence(
        self,
        question_text: str,
        options: dict[str, str] | None = None,
        subject: str = "",
    ) -> dict:
        """Use DeepSeek to solve a choice question with confidence metadata."""
        prompt_parts = []
        if subject:
        ...
        tokens = response.usage.total_tokens if response.usage else "?"
        logger.info(
            f"AI structured answer: reply_len={len(reply)}, tokens={tokens}"
        )
        ...
        valid_answer = answer in valid_answers
        ...
        raw_usable = data.get("usable_for_grading", False)
        if isinstance(raw_usable, str):
            usable_for_grading = raw_usable.strip().lower() in {
                "true",
                "1",
                "yes",
            }
        else:
            usable_for_grading = bool(raw_usable)
        usable_for_grading = usable_for_grading and valid_answer
        ...
        return {
            ...
            "analysis_md": str(
                data.get("analysis_md") or data.get("analysis") or ""
            ).strip(),
            "key_points": key_points,
            "uncertainty_reason": str(
                data.get("uncertainty_reason") or ""
            ).strip(),
        }
