...
- ???? MinerU ??????? PDF ????????? GPU ??????? Markdown??????????
- ???????? `scripts/qc_question_bank.py`??????? Markdown?CSV?JSON ?????????????????????????????
- ??????? MinerU ??? Markdown ?????`<details>` ????Mermaid/text_image ???HTML ???LaTeX array???? LaTeX ???????
- ???????????? A/B/C/D ??????????????????????????????????
- ???????????`2876` ???`195` ?????????????????????? DeepSeek API ?????????????????
...
桌面 Agent 可读取 `exports/agent_notes/manifest.json` 找到最近一次导出，并将
Markdown 笔记同步到 FlowUs、Notion、Obsidian、语雀或本地知识库。后端只负责
生成本地快照，不直接连接第三方笔记软件。
...
1. **使用仓库内置题库** — 在「题库导入 -> 自选题库」中选择 `question_banks/` 内的题库包，导入后写入本地运行时数据库。
2. **导入自己的资料** — 推荐使用 MinerU Markdown，其次是文字型 PDF；扫描型 PDF 属于高级功能，需要配置视觉模型 API 后才能完成图片识别。

内置题库包采用 `question_banks/<bank_id>/metadata.json` + `bank.db` 的目录格式，可包含 `media/`、QC 信息等附加内容；`questions.db` 是旧包兼容格式，仍可导入。
...
├── question_banks/             # 内置题库包 (metadata.json + bank.db)
├── images/
