...
> 不配置也能启动，刷题等核心功能正常使用，仅 AI 助教和扫描型 PDF 导入不可用。
...
> 首次运行需要 `--build` 构建镜像，后续更新代码后也需要加 `--build` 重新构建。

**4. 导入内置题库**

打开浏览器访问 http://localhost:8501，进入「题库导入 → 自选题库」，勾选需要的科目题库包，点击「导入选中题库」。仓库内置 4 个王道科目题库包（共 2872 题 + 195 张图片），clone 后可直接使用。

**5. 开始使用**

| 地址 | 说明 |
|------|------|
| http://localhost:8501 | 前端界面 |
...
适合需要修改代码或调试的开发者。需要 Python 3.12+ 环境。
...
> DeepSeek 不支持图片输入，因此扫描版 PDF 的题目提取需要单独的视觉模型。如果只用文字型 PDF 或内置题库，可以不配视觉模型。
...
启动后自动打开浏览器，访问 http://127.0.0.1:8501。

**5. 导入题库**

在前端「题库导入 → 自选题库」中勾选内置题库包导入，或使用其他 Tab 上传自有资料（Markdown / 文字型 PDF / 扫描型 PDF）。
...
> Docker 部署的完整步骤见上方「快速开始 → 方式一」。这里补充 Docker 特有的注意事项。

**环境变量**：Docker Compose 自动读取项目根目录的 `.env` 文件，将变量注入到容器环境变量中。没有 `.env` 也能启动，但 AI 助教功能会降级（显示"API未配置"提示），不影响刷题和内置题库导入等核心功能。

**数据持久化**：Docker 通过卷挂载持久化以下目录，重启容器不会丢失数据：
...
| `images/` | 题目图片资源 |
| `logs/` | 应用日志 |
| `question_banks/` | 内置题库包 |
| `exports/` | Agent 笔记导出快照 |

**更新代码后**必须重建镜像，否则容器内仍是旧代码：
...
> ⚠️ `docker compose up -d` 不带 `--build` 会使用缓存的旧镜像，导致前端显示旧版本。每次拉取新代码后务必加 `--build`。
